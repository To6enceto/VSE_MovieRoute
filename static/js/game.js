...
let hasUpdatedSpeed = false; 
function updateSpeed(){
    
    if(score % 5 == 0 && score != 0 && !hasUpdatedSpeed){
      speed += 3;
      hasUpdatedSpeed = true;
    } 

    if(score % 5 !== 0 || score === 0){
        hasUpdatedSpeed = false;
    }
}
...
function drawSpeed(){
    ctx.fillStyle = "white";
    ctx.font = "10px Verdana";
    ctx.fillText("speed" + speed, canvas.width - 100, 10);
}
...
document.getElementById('pauseButton').addEventListener('click', function() {
    ...
    if (!isGamePaused) {
        // restart the game loop if coming out of pause
        drawGame();
    }

    this.textContent = isGamePaused ? 'Resume' : 'Pause';
});
